...
export function JobSummaryCard({ job }: JobSummaryCardProps) {
  const channelPreview = job.generatedAds.slice(0, 3).map((ad) => ad.channel);

  return (
    <Card className="talent-card role-summary-card">
      <div className="talent-card-header">
        <div>
          <p className="eyebrow eyebrow-soft">{job.sourceCampaign || "Active role"}</p>
          <h3>{job.title}</h3>
        </div>
      ...
      <div className="talent-card-stats">
        ...
        <div>
          ...
          <strong>{channelPreview.join(" | ") || "Pending"}</strong>
        </div>
      </div>

      <div className="talent-card-actions talent-card-actions-split">
        <span className="muted inline-meta"><Layers3 size={14} /> Posts, applicants, and status live inside the role page.</span>
